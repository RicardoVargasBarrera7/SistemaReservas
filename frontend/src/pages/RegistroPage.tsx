import { CalendarCheck2, CircleCheck, Smartphone } from 'lucide-react'
...
const beneficios = [
  { icono: CalendarCheck2, titulo: 'Reservas rápidas', descripcion: 'Gestiona tus horarios fácilmente.' },
  { icono: CircleCheck, titulo: 'Canchas disponibles', descripcion: 'Consulta horarios y disponibilidad.' },
  { icono: Smartphone, titulo: 'Todo en un solo lugar', descripcion: 'Administra tus reservas desde cualquier dispositivo.' },
]
    ...
    <main className="flex min-h-screen flex-col bg-slate-50 lg:flex-row">
      ...
      >
        <div className="relative z-10 flex items-center gap-3">
          <div className="grid h-12 w-11 place-items-center rounded-[14px_14px_18px_18px] border border-white/70 bg-black/10 shadow-lg backdrop-blur-sm"><span className="text-xl" aria-hidden="true">⚽</span></div>
          <div>
            <strong className="block text-xl font-extrabold tracking-tight sm:text-2xl">Cancha <em className="not-italic text-green-400">Sintética</em></strong>
        ...
        <div className="relative z-10 my-16 max-w-2xl lg:my-auto">
          <p className="mb-7 text-sm font-medium italic text-green-100/80">Tu próxima jugada</p>
          <h1 className="mb-5 text-[clamp(2.25rem,4vw,4rem)] font-extrabold leading-[1.04] tracking-tight">Tu próxima jugada<br /><em className="not-italic text-green-400">comienza aquí.</em></h1>
          <p className="max-w-md text-base leading-relaxed text-slate-200 sm:text-lg">Crea tu cuenta y comienza a reservar tu cancha de forma rápida y sencilla.</p>
        </div>
        ...
        <div className="relative z-10 flex max-w-2xl gap-3 sm:gap-8">
          {beneficios.map(({ icono: Icono, titulo, descripcion }) => (
            <div key={titulo} className="flex min-w-0 flex-1 items-start gap-2 border-r border-white/20 pr-3 last:border-0 sm:gap-3 sm:pr-7">
              ...
              <div className="grid gap-0.5"><strong className="text-xs font-bold sm:text-sm">{titulo}</strong><small className="text-[11px] text-green-100/70 sm:text-xs">{descripcion}</small></div>
            </div>
          ))}
        </div>
